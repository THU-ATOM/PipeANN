...
float (*atofLambda)(const std::string &) = [](const std::string &str) { return (float) ::atof(str.c_str()); };
int8_t (*atoiLambda)(const std::string &) = [](const std::string &str) { return (int8_t)::atoi(str.c_str()); };
uint8_t (*atouLambda)(const std::string &) = [](const std::string &str) { return (uint8_t)::atoi(str.c_str()); };

const int MIN_POINTS_FOR_VALUE_VERIFICATION = 10000;

std::vector<std::string> splitLine(const std::string &str, const char delimiter, int suggestedCapacity) {
...
template<typename T>
std::vector<T> getVector(const std::string &str, T convertFn(const std::string &str), int suggestedSize) {
  ...
  if (idsAndVector.size() >= 3) {
    ...
    std::cerr << "Found line " << str << " that has non-standard number of delimiters: " << idsAndVector.size()
              << std::endl;
  }
  std::transform(strVec.begin(), strVec.end(), std::back_inserter(typedVec),
...
uint32_t getDimension(const char *inFile, T convertFn(const std::string &val)) {
  ...
  if (fin.is_open()) {
    ...
    char message[2048];
    std::cerr << "Could not open input file: " << inFile << ". " << strerror_s(message, 2048, errno) << std::endl;
    exit(1);
  }
}
...
uint32_t getRowCount(const char *infile) {
  ...
  while (std::getline(fin, line)) {
    if (line.length() != 0) {
      count++;
      if (count % 10000 == 0) {
        diskann::cout << "Counted " << count << " lines." << std::endl;
      }
    }
  }
...
template<typename T>
void writeBinaryVec(const char *outfile, uint32_t numDimensions, const std::vector<std::vector<T>> &allVecs) {
  uint32_t numPoints = (uint32_t) allVecs.size();
  diskann::cout << "Writing  " << numPoints << " records of dimension: " << numDimensions << " to file: " << outfile
                << "...";
  ...
  if (fout.is_open()) {
    ...
    char message[2048];
    std::cerr << "Could not open output file: " << outfile << ". " << strerror_s(message, 2048, errno) << std::endl;
  }
}
...
uint32_t numPointsToCheck(uint32_t numPoints) {
  ...
  return point01percent > MIN_POINTS_FOR_VALUE_VERIFICATION
             ...
             : point1percent > MIN_POINTS_FOR_VALUE_VERIFICATION
                   ? point1percent
                   : onepercent > MIN_POINTS_FOR_VALUE_VERIFICATION ? onepercent : numPoints;
}

template<typename T>
void sanityCheck(const char *origfile, const char *binfile, uint32_t numDimensions, uint32_t numPoints,
  ...
  assert(np == numPoints);
  if (np != numPoints) {
    throw diskann::ANNException("Verifying binary write failed! np != numPoints", -1, __FUNCSIG__, __FILE__, __LINE__);
  }
  assert(nd == numDimensions);
  if (nd != numDimensions) {
    throw diskann::ANNException("Verifying binary write failed! nd != numDimensions", -1, __FUNCSIG__, __FILE__,
                                __LINE__);
  }
  ...
  diskann::cout << "Checking first " << vectorsToCheck << " records for byte-byte match." << std::endl;
  ...
  for (uint32_t i = 0; i < vectorsToCheck; i++) {
    ...
    for (uint32_t j = 0; j < nd; j++) {
      if (origvec[j] != data[i * nd + j]) {
        std::cerr << "Found mismatch at line " << i << " orig: " << origvec[j] << " bin val: " << data[i * nd + j]
                  << std::endl;
        mismatchCount++;
      }
    }
  ...
  if (mismatchCount == 0) {
    diskann::cout << "Successfully verified byte-byte conversion." << std::endl;
  } else {
    diskann::cout << "Conversion failed!!! Found " << mismatchCount << " mismatches in vector elements" << std::endl;
  }
  diskann::cout << "Successfully verified binary conversion." << std::endl;
}

template<typename T>
std::vector<T> getVector(const std::string &str, T convertFn(const std::string &val)) {
  auto idsAndVector = splitLine(str, '\t', suggestedSize);
  ...
  if (idsAndVector.size() >= 3) {
    ...
    std::cerr << "Found line " << str << " that didn't have standard delimiters." << std::endl;
  }
  return typedVec;
}

template<typename T>
void convert(const char *infile, const char *outfile, T convertFn(const std::string &val), uint32_t suggestedSize) {
  ...
  while (std::getline(fin, line)) {
    ...
    if (vector.size() != 0) {
      if (vector.size() != numDimensions) {
        ...
        stream << "Found vector with dimension " << vector.size() << " instead of " << numDimensions
               << " at line: " << lc;
        throw diskann::ANNException(stream.str(), -1);
      }
      allVecs.push_back(vector);
      if (allVecs.size() % 10000 == 0) {
        diskann::cout << "Processed " << allVecs.size() << " records." << std::endl;
      }
  ...
  sanityCheck<T>(infile, outfile, numDimensions, (uint32_t) allVecs.size(), convertFn);
}
...
int main(int argc, char **argv) {
  ...
  diskann::cout << "Converting (" << argv[1] << ") axon file: " << argv[2] << " with " << argv[4]
                << " points to diskann bin format file " << argv[3] << std::endl;
  ...
  if (datatype == "uint8") {
    ...
    std::cerr << "Unknown data type " << argv[1] << ". Supported data types are int8, uint8, and float." << std::endl;
  }
}