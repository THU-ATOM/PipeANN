...
template<typename T>
void extract_rows(const std::string &input_file, _u64 nrows, _u64 ndims, _u64 offset, _u64 nrows_to_extract,
                  const std::string &output_file, bool has_id, bool replace_content) {
  std::cout << "Trying to extract: " << nrows_to_extract << " rows of " << typeid(T).name() << " data in " << ndims
            << " dimensions from offset: " << offset << " and saving to file: " << output_file << "...";
  std::ifstream fin;
  ...
  if (has_id) {
    ...
    _u64 tag_start = 2 * sizeof(_u32) + nrows * ndims * sizeof(T) + offset * sizeof(_u64);
    fin.seekg(tag_start);
...
int main(int argc, char *argv[]) {
  if (argc != 6 && argc != 7 && argc != 8) {
    std::cout << "Usage: <program> <input_file> <datatype> (uint8|int8|float) "
  ...
  if (!file_exists(input_file)) {
    std::cerr << "Input file: " << input_file << " does not exist. Terminating." << std::endl;
    return -1;
  }
  ...
  if (nrows_to_extract > nrows) {
    std::cerr << "Number of rows in file: " << nrows << " is less than num rows to extract: " << nrows_to_extract
              << std::endl;
    return -2;
  }
  if (offset > nrows) {
    std::cerr << "Start offset " << offset << " is greater than or equal to rows in file: " << nrows << std::endl;
    return -3;
  }
  if (offset + nrows_to_extract > nrows) {
    std::cerr << "Sum of start offset: " << offset << " and # rows to extract: " << nrows_to_extract
              << " is greater than or equal to rows in file: " << nrows << std::endl;
    return -4;
  }

  if (datatype == "uint8") {
    extract_rows<uint8_t>(input_file, nrows, ncols, offset, nrows_to_extract, output_file, has_id, replace_content);
  } else if (datatype == "int8") {
    extract_rows<int8_t>(input_file, nrows, ncols, offset, nrows_to_extract, output_file, has_id, replace_content);
  } else if (datatype == "float") {
    extract_rows<float>(input_file, nrows, ncols, offset, nrows_to_extract, output_file, has_id, replace_content);
