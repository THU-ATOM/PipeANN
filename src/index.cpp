...
namespace diskann {
  ...
  Index<T, TagT>::Index(Metric m, const size_t dim, const size_t max_points, const bool dynamic_index,
                        const bool save_index_in_one_file, const bool enable_tags, const bool support_eager_delete)
      : _dist_metric(m), _dim(dim), _max_points(max_points), _save_as_one_file(save_index_in_one_file),
        _dynamic_index(dynamic_index), _enable_tags(enable_tags), _support_eager_delete(support_eager_delete) {
    if (dynamic_index && !enable_tags) {
      throw diskann::ANNException("ERROR: Eager Deletes must have Dynamic Indexing enabled.", -1, __FUNCSIG__, __FILE__,
                                  __LINE__);
      diskann::cerr << "WARNING: Dynamic Indices must have tags enabled. Auto-enabling." << std::endl;
      _enable_tags = true;
    }
    ...
    diskann::cout << "Getting distance function for metric: " << (m == diskann::Metric::COSINE ? "cosine" : "l2")
                  << std::endl;
  ...
  void Index<T, TagT>::save(const char *filename) {
    ...
    if (!_save_as_one_file) {
      ...
      diskann::save_bin<_u64>(filename, cumul_bytes.data(), cumul_bytes.size(), 1, 0);

      diskann::cout << "Saved index as one file to " << filename << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                    << "B." << std::endl;
    }
    ...
    diskann::cout << "Time taken for save: " << timespan.count() << "s." << std::endl;
  }
  ...
  void Index<T, TagT>::load(const char *filename) {
    ...
    if (data_file_num_pts != graph_num_pts || (data_file_num_pts != tags_file_num_pts && _enable_tags)) {
      std::stringstream stream;
      stream << "ERROR: When loading index, loaded " << data_file_num_pts << " points from datafile, " << graph_num_pts
             << " from graph, and " << tags_file_num_pts << " tags, with num_frozen_pts being set to "
             << _num_frozen_pts << " in constructor." << std::endl;
      std::cerr << stream.str() << std::endl;
      aligned_free(_data);
      throw diskann::ANNException(stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    reposition_frozen_point_to_end();
    diskann::cout << "Num frozen points:" << _num_frozen_pts << " _nd: " << _nd << " _ep: " << _ep
  ...
  void Index<T, TagT>::load_from_disk_index(const std::string &filename) {
    ...
    char *buf;
    diskann::alloc_aligned((void **) &buf, kSectorsPerRead * kSectorLen, kSectorLen);
...
#pragma omp critical
      ...
      LOG(INFO) << "Loaded " << (in_sector + 1) * nnodes_per_sector << "/" << disk_nnodes << " nodes.";
    }
  ...
  size_t Index<T, TagT>::load_graph(std::string filename, size_t expected_num_points, size_t offset) {
    ...
    while (bytes_read != expected_file_size) {
      ...
      if (k == 0) {
        diskann::cerr << "ERROR: Point found with no out-neighbors, point#" << nodes << std::endl;
      }
      //      if (in.eof())
      //        break;
      cc += k;
    ...
    diskann::cout << "done. Index has " << nodes << " nodes and " << cc << " out-edges, _ep is set to " << _ep
                  << std::endl;
    return nodes;
  }
  ...
  const T *Index<T, TagT>::get_vector_by_tag(const TagT &tag) {
    std::shared_lock<std::shared_timed_mutex> lock(_tag_lock);
    if (_tag_to_location.find(tag) == _tag_to_location.end()) {
      diskann::cout << "Tag " << tag << " does not exist in the index." << std::endl;
      return nullptr;
    } else {
      unsigned location = _tag_to_location[tag];
...
#pragma omp parallel for schedule(static, 65536)
      ...
      if (best_L_nodes[k].flag) {
        ...
        if (_dynamic_index) {
          ...
          for (unsigned m = 0; m < _final_graph[n].size(); m++) {
            if (_final_graph[n][m] >= _max_points + _num_frozen_pts) {
              diskann::cerr << "Wrong id found: " << _final_graph[n][m] << std::endl;
              throw diskann::ANNException(std::string("Wrong id found") + std::to_string(_final_graph[n][m]), -1,
        ...
        for (unsigned m = 0; m < des.size(); ++m) {
          ...
          if (inserted_into_pool.find(id) == inserted_into_pool.end()) {
            ...
            if ((m + 1) < des.size()) {
              auto nextn = des[m + 1];
              diskann::prefetch_vector((const char *) _data + _aligned_dim * (size_t) nextn, sizeof(T) * _aligned_dim);
            }
   ...
   */
  template<typename T, typename TagT>
  void Index<T, TagT>::batch_inter_insert(unsigned n, const std::vector<unsigned> &pruned_list,
    ...
    for (auto des : pruned_list) {
...

      {
        ...
        if (std::find(_final_graph[des].begin(), _final_graph[des].end(), n) == _final_graph[des].end()) {
...
#pragma omp parallel for schedule(dynamic)
        for (_s64 node_ctr = (_s64) start_id; node_ctr < (_s64) end_id; ++node_ctr) {
...
#pragma omp parallel for schedule(dynamic, 65536)
        ...
        if ((sync_num * 100) / NUM_SYNCS > progress_counter) {
          ...
          diskann::cout << "Completed  (round: " << rnd_no << ", sync: " << sync_num << "/" << NUM_SYNCS << " with L "
                        << L << ")"
      ...
      if (_nd > 0) {
        diskann::cout << "Completed Pass " << rnd_no << " of data using L=" << L
                      << " and alpha=" << parameters.Get<float>("alpha") << ". Stats: ";
        diskann::cout << "search+prune_time=" << total_sync_time << "s, inter_time=" << total_inter_time
...
#pragma omp parallel for
    for (_s64 node = 0; node < (_s64)(_max_points + _num_frozen_pts); node++) {
      if ((size_t) node < _nd || (size_t) node == _max_points) {
        if (_final_graph[node].size() > range) {
          ...
          prune_neighbors((_u32) node, dummy_pool, parameters, new_out_neighbors);
  ...
  void Index<T, TagT>::build(const char *filename, const size_t num_points_to_load, Parameters &parameters,
    ...
    if (!file_exists(filename)) {
      diskann::cerr << "Data file " << filename << " does not exist!!! Exiting...." << std::endl;
      std::stringstream stream;
    ...
    if (filename == nullptr) {
      ...
      diskann::cout << "Loading only first " << num_points_to_load << " from file.. " << std::endl;
      _nd = num_points_to_load;

      if (_enable_tags && tags.size() != num_points_to_load) {
        std::stringstream stream;
        stream << "ERROR: Driver requests loading " << num_points_to_load << " points from file,"
  ...
  void Index<T, TagT>::build(const char *filename, const size_t num_points_to_load, Parameters &parameters,
    ...
    if (!file_exists(filename)) {
      std::cerr << "Data file provided " << filename << " does not exist." << std::endl;
      std::stringstream stream;
      stream << "Data file provided " << filename << " does not exist." << std::endl;
      std::cerr << stream.str() << std::endl;
      throw diskann::ANNException(stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
      ...
      if (_enable_tags) {
        if (tag_filename == nullptr) {
          ...
          if (file_exists(tag_filename)) {
            ...
            diskann::cerr << "Tag file " << tag_filename << " does not exist. Exiting..." << std::endl;
  ...
  std::pair<uint32_t, uint32_t> Index<T, TagT>::search(const T *query, const size_t K, const unsigned L,
    ...
    for (auto iter : best) {
      if (_location_to_tag.find(iter.id) != _location_to_tag.end())
        best_K_tags.emplace_back(Neighbor_Tag<TagT>(_location_to_tag[iter.id], iter.distance));
  ...
  template<typename T, typename TagT>
  int Index<T, TagT>::eager_delete(const TagT tag, const Parameters &parameters, int delete_mode) {
...

      {
        ...
        {  // should a shared reader lock on delete_lock be held here at the
          ...
          for (auto j : _final_graph[id]) {
            if ((j != id) && (j != ngh) && (_delete_set.find(j) == _delete_set.end()))
        ...
        for (auto j : candidate_set)
          expanded_nghrs.push_back(
              Neighbor(j,
                       _distance->compare(_data + _aligned_dim * (size_t) ngh, _data + _aligned_dim * (size_t) j,
          ...
          for (size_t i = 0; i < result.size(); i++) {
            ...
            if (_delete_set.find(j.id) == _delete_set.end()) {
              _final_graph[ngh].push_back(j.id);
              {
                ...
                if (std::find(_in_graph[j.id].begin(), _in_graph[j.id].end(), ngh) == _in_graph[j.id].end()) {
  ...
  void Index<T, TagT>::update_in_graph() {
    ...
    for (size_t i = 0; i < _final_graph.size(); i++)  // copying to in-neighbor graph
  ...
  size_t Index<T, TagT>::consolidate_deletes(const Parameters &parameters) {
    if (_eager_done) {
      diskann::cout << "In consolidate_deletes(), _eager_done is true. So exiting." << std::endl;
      return 0;
    }

    diskann::cout << "Inside Index::consolidate_deletes()" << std::endl;
    std::cout << "Empty slots size: " << _empty_slots.size() << " _nd: " << _nd << " max_points: " << _max_points
              << std::endl;
    assert(_enable_tags);
...
#pragma omp parallel for schedule(dynamic)
    ...
    std::cout << "Time taken for consolidate_deletes() "
              << std::chrono::duration_cast<std::chrono::duration<double>>(stop - start).count() << "s." << std::endl;
  ...
  void Index<T, TagT>::compact_data() {
    ...
    if (_delete_set.find(_ep) != _delete_set.end()) {
      std::cerr << "Replacing start node which has been deleted... " << std::flush;
      auto old_ep = _ep;
      ...
      if (_ep == old_ep) {
        diskann::cerr << "ERROR: Did not find a replacement for start node." << std::endl;
        throw diskann::ANNException("ERROR: Did not find a replacement for start node.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
      } else {
        ...
        for (size_t i = 0; i < _final_graph[old].size(); ++i) {
          if (new_location[_final_graph[old][i]] > _final_graph[old][i]) {
            ...
            if (_delete_set.find(_final_graph[old][i]) != _delete_set.end()) {
              sstream << std::endl
                      << " Point: " << old << " index: " << i << " neighbor: " << _final_graph[old][i]
                      << " found in delete set of size: " << _delete_set.size() << std::endl;
            } else {
              sstream << " Point: " << old << " neighbor: " << _final_graph[old][i]
                      << " NOT found in delete set of size: " << _delete_set.size() << std::endl;
            }
    ...
    std::cout << "Time taken for compact_data(): "
              << std::chrono::duration_cast<std::chrono::duration<double>>(stop - fnstart).count() << "s." << std::endl;
  }
  ...
  void Index<T, TagT>::printTagToLocation() {
    ...
    std::cout << "Thread: " << std::this_thread::get_id() << " _tag_to_location: " << std::endl;
    for (auto tl : _tag_to_location) {
      std::cout << "(" << tl.first << "," << tl.second << "),";
    }
    std::cout << std::endl << "Thread: " << std::this_thread::get_id() << " _location_to_tag: " << std::endl;
  ...
  int Index<T, TagT>::insert_point(const T *point, const Parameters &parameters, const TagT tag) {
    ...
    if (location == -1) {
      std::cout << "Thread: " << std::this_thread::get_id() << " location  == -1. Waiting for unique_lock. "
                << std::endl
                << std::flush;
      lock.unlock();
      std::unique_lock<std::shared_timed_mutex> growth_lock(_update_lock);
      ...
      if (location == -1) {
        throw diskann::ANNException("Cannot reserve location even after expanding graph. Terminating.", -1, __FUNCSIG__,
                                    __FILE__, __LINE__);
      }
    }
      ...
      for (unsigned i = 0; i < _final_graph[location].size(); i++) {
        {
          ...
          _in_graph[_final_graph[location][i]].erase(std::remove(_in_graph[_final_graph[location][i]].begin(),
  ...
  template<typename T, typename TagT>
  int Index<T, TagT>::extract_data(T *ret_data, std::unordered_map<TagT, unsigned> &tag_to_location) {
    if (!_data_compacted) {
      std::cerr << "Error! Data not compacted. Cannot give access to private data." << std::endl;
      return -1;
    }
  ...
  template<typename T, typename TagT>
  void Index<T, TagT>::get_location_to_tag(std::unordered_map<unsigned, TagT> &ret_loc_to_tag) {
  ...
  void Index<T, TagT>::print_delete_set() const {
    diskann::cout << "Delete set is of size: " << _delete_set.size() << std::endl;
    ...
    diskann::cout << "Sorted Delete set is of size: " << _delete_set.size() << std::endl;
    ...
    diskann::cout << "Missing ids in delete set for the 60k-80k case:" << std::endl;
    ...
    for (size_t i = 0; i < _nd; i++) {
      for (size_t j = 0; j < _final_graph[i].size(); j++) {
        if (_delete_set.find(_final_graph[i][j]) != _delete_set.end()) {
          start_end_pairs.push_back(std::pair<_u32, _u32>(i, _final_graph[i][j]));
        }
      }
    }

    if (start_end_pairs.size() > 0) {
      diskann::cout << "Found " << start_end_pairs.size() << " references to deleted vertices" << std::endl;
      std::sort(
          start_end_pairs.begin(), start_end_pairs.end(),
          [](const std::pair<_u32, _u32> &val1, const std::pair<_u32, _u32> &val2) { return val1.first < val2.first; });
      diskann::cout << "Min source id: " << start_end_pairs[0].first
                    << " Max source id: " << start_end_pairs[start_end_pairs.size() - 1].first << std::endl;
      std::sort(start_end_pairs.begin(), start_end_pairs.end(),
                [](const std::pair<_u32, _u32> &val1, const std::pair<_u32, _u32> &val2) {
                  return val1.second < val2.second;
                });
      diskann::cout << "Min target id: " << start_end_pairs[0].second
                    << " Max target id: " << start_end_pairs[start_end_pairs.size() - 1].second << std::endl;
    }
  }
  ...
  void Index<T, TagT>::print_status() const {
    ...
    diskann::cout << std::boolalpha << "Data compacted: " << this->_data_compacted << " Lazy done: " << this->_lazy_done
