...
namespace diskann {
  ...
  double calculate_recall(unsigned num_queries, unsigned *gold_std, float *gs_dist, unsigned dim_gs,
    ...
    for (size_t i = 0; i < num_queries; i++) {
      ...
      if ((active_points_count < recall_at && !active_tags.empty()) && !printed) {
        diskann::cout << "Warning: Couldn't find enough closest neighbors " << active_points_count << "/" << recall_at
                      << " from "
                         "truthset for query # "
  ...
  template<typename T>
  int build_merged_vamana_index(std::string base_file, diskann::Metric _compareMetric, bool single_file_index,
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      diskann::cout << "Full index fits in RAM, building in one shot" << std::endl;
      ...
      std::unique_ptr<diskann::Index<T>> _pvamanaIndex = std::unique_ptr<diskann::Index<T>>(
          new diskann::Index<T>(_compareMetric, base_dim, base_num, false, single_file_index, tags_enabled));
      if (tags_enabled)
    ...
    for (int p = 0; p < num_parts; p++) {
      ...
      std::unique_ptr<diskann::Index<T>> _pvamanaIndex = std::unique_ptr<diskann::Index<T>>(
          new diskann::Index<T>(_compareMetric, shard_base_dim, shard_base_pts, false,
  ...
  void create_disk_layout(const std::string &mem_index_file, const std::string &base_file, const std::string &tag_file,
    ...
    if (single_file_index) {
      ...
      if (nr != Index<T, TagT>::METADATA_ROWS && nc != 1) {
        std::stringstream stream;
        stream << "Vamana Single Index file size does not meet meta-data criteria." << std::endl;
    ...
    for (_u64 sector = 0; sector < n_sectors; sector++) {
      ...
      for (_u64 sector_node_id = 0; sector_node_id < nnodes_per_sector && cur_node_id < npts_64; sector_node_id++) {
        ...
        if (nnbrs == 0) {
          diskann::cout << "ERROR. Found point with no out-neighbors; Point#: " << cur_node_id << std::endl;
          exit(-1);
        }
    ...
    if (vamana_frozen_num > 0) {
      ...
      size_t nr, nc;
      if (single_file_index)
        diskann::load_bin<TagT>(mem_index_file, mem_index_tags, nr, nc, tags_offset);
      else
        diskann::load_bin<TagT>(tag_file, mem_index_tags, nr, nc, tags_offset);

      if (nr != npts_64 && nc != 1) {
        std::stringstream stream;
        stream << "Error loading tags file. File dims are " << nr << ", " << nc << ", but expecting " << npts_64
      ...
      if (single_file_index) {
        tag_bytes_written = diskann::save_bin<TagT>(output_file, mem_index_tags.get(), nr, nc,
                                                    output_file_meta[output_file_meta.size() - 1]);
      } else {
        diskann::save_bin<TagT>(output_file + std::string(".tags"), mem_index_tags.get(), nr, nc);
      }
    } else {
      if (tags_enabled) {
        std::unique_ptr<TagT[]> mem_index_tags;
        ...
        if (single_file_index) {
          ...
          if (!file_exists(tag_file)) {
            diskann::cout << "Static vamana index, tag file " << tag_file << "does not exist. Exiting...." << std::endl;
            exit(-1);
          }
  ...
  bool build_disk_index(const char *dataPath, const char *indexFilePath, const char *indexBuildParameters,
                        diskann::Metric _compareMetric, bool single_file_index, const char *tag_file) {
    ...
    if (indexing_ram_budget <= 0) {
      diskann::cerr << "Not building index. Please provide more RAM budget" << std::endl;
      return false;
    }
    ...
    diskann::cout << "Starting index build: R=" << R << " L=" << L << " Query RAM budget: " << final_index_ram_limit
                  ...
                  << " Final index will be in " << (single_file_index ? "single file" : "multiple files") << std::endl;
    ...
    diskann::cout << "(Normalized, if required) file : " << normalized_file_path << " has: " << points_num
                  << " points. Changing training set size to " << training_set_size << " points" << std::endl;
    ...
    diskann::cout << "Generating PQ pivots with training data of size: " << train_size
                  << " num PQ chunks: " << num_pq_chunks << std::endl;
    generate_pq_pivots(train_data, train_size, (uint32_t) dim, 256, (uint32_t) num_pq_chunks, NUM_KMEANS,
                       pq_pivots_path);
    auto end = std::chrono::high_resolution_clock::now();

    diskann::cout << "Pivots generated in " << std::chrono::duration<double>(end - start).count() << "s." << std::endl;
    ...
    diskann::cout << "Compressed data generated and written in: " << std::chrono::duration<double>(end - start).count()
                  << "s." << std::endl;
    start = std::chrono::high_resolution_clock::now();
    diskann::build_merged_vamana_index<T>(normalized_file_path, _compareMetric, single_file_index, L, R, p_val,
                                          indexing_ram_budget, mem_index_path, medoids_path, centroids_path, tag_file);
    end = std::chrono::high_resolution_clock::now();
    diskann::cout << "Vamana index built in: " << std::chrono::duration<double>(end - start).count() << "s."
                  << std::endl;

    if (tag_file == nullptr) {
      diskann::create_disk_layout<T, TagT>(mem_index_path, normalized_file_path, "", pq_pivots_path,
                                           pq_compressed_vectors_path, single_file_index, disk_index_path);
    } else {
      std::string tag_filename = std::string(tag_file);
      diskann::create_disk_layout<T, TagT>(mem_index_path, normalized_file_path, tag_filename, pq_pivots_path,
                                           pq_compressed_vectors_path, single_file_index, disk_index_path);
    }
    ...
    diskann::cout << "Generating warmup file with " << num_sample_points
                  << " points using a sampling rate of: " << sample_sampling_rate << std::endl;
    gen_random_slice<T>(normalized_file_path, sample_base_prefix, sample_sampling_rate);

    diskann::cout << "Deleting memory index file: " << mem_index_path << std::endl;
    ...
    if (normalized_file_path != dataFilePath) {
      ...
      diskann::cout << "Deleting normalized vector file: " << normalized_file_path << std::endl;
  ...
  template DISKANN_DLLEXPORT bool build_disk_index<int8_t, uint32_t>(const char *dataFilePath,
                                                                     const char *indexFilePath,
                                                                     const char *indexBuildParameters,
                                                                     diskann::Metric _compareMetric,
                                                                     bool singleFileIndex, const char *tag_file);
  template DISKANN_DLLEXPORT bool build_disk_index<uint8_t, uint32_t>(const char *dataFilePath,
                                                                      const char *indexFilePath,
                                                                      const char *indexBuildParameters,
                                                                      diskann::Metric _compareMetric,
                                                                      bool singleFileIndex, const char *tag_file);
  template DISKANN_DLLEXPORT bool build_disk_index<float, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                                    const char *indexBuildParameters,
                                                                    diskann::Metric _compareMetric,
                                                                    bool singleFileIndex, const char *tag_file);
  // template DISKANN_DLLEXPORT bool build_disk_index<int8_t, uint64_t>(const char *dataFilePath,
  //                                                                    const char *indexFilePath,
  //                                                                    const char *indexBuildParameters,
  //                                                                    diskann::Metric _compareMetric,
  //                                                                    bool singleFileIndex, const char *tag_file);
  // template DISKANN_DLLEXPORT bool build_disk_index<uint8_t, uint64_t>(const char *dataFilePath,
  //                                                                     const char *indexFilePath,
  //                                                                     const char *indexBuildParameters,
  //                                                                     diskann::Metric _compareMetric,
  //                                                                     bool singleFileIndex, const char *tag_file);
  // template DISKANN_DLLEXPORT bool build_disk_index<float, uint64_t>(const char *dataFilePath, const char
  // *indexFilePath,
  //                                                                   const char *indexBuildParameters,
  //                                                                   diskann::Metric _compareMetric,
  //                                                                   bool singleFileIndex, const char *tag_file);
  ...
  template DISKANN_DLLEXPORT int build_merged_vamana_index<int8_t>(std::string base_file,
                                                                   diskann::Metric _compareMetric,
                                                                   bool single_file_index, unsigned L, unsigned R,
  ...
  template DISKANN_DLLEXPORT int build_merged_vamana_index<float>(std::string base_file, diskann::Metric _compareMetric,
                                                                  bool single_file_index, unsigned L, unsigned R,
                                                                  double sampling_rate, double ram_budget,
                                                                  std::string mem_index_path, std::string medoids_path,
                                                                  std::string centroids_file, const char *tag_file);
  template DISKANN_DLLEXPORT int build_merged_vamana_index<uint8_t>(
      std::string base_file, diskann::Metric _compareMetric, bool single_file_index, unsigned L, unsigned R,
